...
(define the-grammar
  '((program (expression) a-program)
    ...
    (expression ("unpair" my-identifier my-identifier "=" expression "in" expression) unpair-exp)))
                      ...
                      (pair-type (ty1 ty2)
                                 (type-of body
                                          (extend-tenv*
                                           ...
                                           (list ty1 ty2)
                                           tenv)))
                      (else (eopl:error 'unpair-exp "unpair-exp-error")))))))


;; (run "unpair num1 num2 = newpair(1,2) in -(num1,num2)")

;; (run "letrec int fn(p:pairof int * int) = unpair num1 num2 = p in -(num1,num2) in (fn newpair(1,2))")