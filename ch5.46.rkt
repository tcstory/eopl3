...
(define (run string)
  (value-of-program 22 (scan&parse string)))
  ...
  (cases expression exp
    ...
    (yield-exp ()
               (let ((time-slice the-time-remaining))
                 (place-on-ready-queue!
                  (lambda ()
                    ;; 本来还想定义一个 thread 的数据结构来存 time-sclie, 后来才发现更简单的方法
                    (set! the-time-remaining time-slice)
                    (apply-cont cont (num-val 99))))
                 (run-next-thread)))))

(run
   "let incr_x = proc(id) proc(dummy)
      begin print(id); if zero?(-(id, 100)) then begin yield(); print(-(0,id)) end else print(-(0,id)) end
