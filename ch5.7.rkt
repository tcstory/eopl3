...
(define (apply-cont cont val)
  (cases continuation cont
    ...
    (let-exp-cont (vars exps body new-env old-env saved-cont)
                  (let ((my-env (extend-env (car vars) val new-env)))
                    (if (= (length vars) 1)
                        (value-of/k body my-env saved-cont)
                        (value-of/k (car exps) old-env
                                  (let-exp-cont (cdr vars) (cdr exps) body my-env old-env saved-cont)))))
