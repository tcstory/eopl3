...
(define the-grammar
  '((program (statement) a-program)
    ...
    (statement (my-identifier "=" expression) assign-stmt)
    (statement ("print" expression) print-stmt)
    (statement ("{" (separated-list statement ";") "}") block-stmt)
    ...
    (statement ("var" (separated-list my-identifier ",") ";" statement) var-stmt)))
...
(define (value-of-program pgm)
  (initialize-store!)
  (cases program pgm
    (a-program (exp1) (result-of exp1 (init-env)))))

(define (block-stmt-val stmts env)
  (if (not (null? stmts))
      (begin
        (result-of (car stmts) env)
        (block-stmt-val (cdr stmts) env))
      (bool-val #t)))

(define (do-while-stmt-val exp1 stmt1 env)
  (result-of stmt1 env)
  (if (expval->bool (value-of exp1 env))
      (do-while-stmt-val exp1 stmt1 env)
      (bool-val #f)))

(define (while-stmt-val exp1 stmt1 env)
  (if (expval->bool (value-of exp1 env))
      (begin
        (result-of stmt1 env)
        (while-stmt-val exp1 stmt1 env))
      (bool-val #f)))

(define (var-stmt-val vars stmt1 env)
  (if (null? vars)
      (result-of stmt1 env)
      (var-stmt-val
       (cdr vars)
       stmt1
       (extend-env (car vars) (newref "undefined") env))))

(define (result-of stmt env)
  (cases statement stmt
    (print-stmt (exp1) (begin
                         (display "[value] ")
                         (display (value-of exp1 env))
                         (newline)))
    (block-stmt (stmts) (block-stmt-val stmts env))
    (if-stmt (exp1 stmt1 stmt2)
             (let ((val0 (value-of exp1 env)))
               (if (expval->bool val0)
                   (result-of stmt1 env)
                   (result-of stmt2 env))))
