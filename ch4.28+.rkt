...
(define the-grammar
  '((program (expression) a-program)
    ...
    (expression ("newarray" "(" expression "," expression ")" ) array-exp)
...
(define (array-length arr)
  (cases array arr
    (array-v (head size) size)))

;; 书上使用的是嵌套的 let 表达式来返回 ref1, 我猜测原因是某些实现中 let 的执行顺序是不确定的
...
(define (make-array-value size val)
  (letrec ((loop (lambda (size0)
                   (if (> size0 0)
                       (begin
                         (newref val)
                         (loop (- size0 1)))
...
(define array-chk
  (lambda (arr pos)
    (cases array arr
	   (array-v (header count)
		    (if (>= pos count)
			(report-expval-extractor-error 'array-chk "index out of range for array")
			#t)))))
...
(define (value-of exp env)
  (cases expression exp
    ...
    (array-exp (exp1 exp2)
               (let ((val1 (value-of exp1 env))
                     (val2 (value-of exp2 env)))
                 (array-val (make-array-value (expval->num val1) val2))))
