...
(define the-grammar
  '((program ((arbno class-decl) expression) a-program)
    ...
    (expression
     ...
     fieldset-exp)
    (expression
     ("superfieldref" expression my-identifier)
     superfieldref-exp)
    (expression
     ("superfieldset" expression my-identifier "=" expression )
     superfieldset-exp)
    (expression
     ("named-send" my-identifier expression my-identifier "(" (separated-list expression ",") ")")
     named-method-call-exp)
    (expression
     ("named-fieldref" my-identifier expression my-identifier)
     named-fieldref-exp)
    (expression
     ("named-fieldset" my-identifier expression my-identifier "=" expression )
     named-fieldset-exp)))
...
(define (get-shadowed-field-name name field)
  (string->symbol
   (string-append
    (symbol->string field)
    "%"
...
(define (append-field-names super-name super-fields new-fields)
  (cond
    ((null? super-fields) new-fields)
    (else
     (cons
      (if (memq (car super-fields) new-fields)
          (get-shadowed-field-name super-name (car super-fields))
          (car super-fields))
      (append-field-names
       super-name
       (cdr super-fields)
    ...
    (fieldset-exp (exp1 field-name exp2)
                  (let* ((obj (value-of exp1 env))
                    ...
                    (if idx
                        ...
                        (eopl:error 'fieldref-exp "not found field ~s." field-name))))
    (superfieldref-exp (exp1 field-name)
                       (let* ((obj (value-of exp1 env))
                              (c-name (object->class-name obj))
                              (s-name (class->super-name (lookup-class c-name)))
                              (fields (class->field-names (lookup-class c-name)))
                              (idx (element-index (get-shadowed-field-name s-name field-name) fields)))
                         (if idx
                             (deref (list-ref (object->fields obj) idx))
                             (eopl:error 'fieldref-exp "not found field ~s." field-name))))
    (superfieldset-exp (exp1 field-name exp2)
                       (let* ((obj (value-of exp1 env))
                              (c-name (object->class-name obj))
                              (s-name (class->super-name (lookup-class c-name)))
                              (fields (class->field-names (lookup-class c-name)))
                         ...
                         (if idx
                             (setref! (list-ref (object->fields obj) idx) (value-of exp2 env))
                             (eopl:error 'fieldref-exp "not found field ~s." field-name))))
    (named-method-call-exp (s-name exp1 method-name rands)
                           (let ((args (values-of-exps rands env))
                                 (obj (value-of exp1 env)))
                             (apply-method
                              (find-method
                               s-name
                               method-name)
                              obj
                              args)))
    (named-fieldref-exp (s-name exp1 field-name)
                        (let* ((obj (value-of exp1 env))
                               (c-name (object->class-name obj))
                               (fields (class->field-names (lookup-class c-name)))
                               (idx (element-index (get-shadowed-field-name s-name field-name) fields)))
                          (if idx
                              (deref (list-ref (object->fields obj) idx))
                              (eopl:error 'fieldref-exp "not found field ~s." field-name))))
    (named-fieldset-exp (s-name exp1 field-name exp2)
                        (let* ((obj (value-of exp1 env))
                               (c-name (object->class-name obj))
                               (fields (class->field-names (lookup-class c-name)))
                               (idx (element-index (get-shadowed-field-name s-name field-name) fields)))
...
;class c3 extends c2
;field c12
;method initialize () begin super initialize(); set c12 = 1200 end
;
;let o3 = new c3() in begin
;log(fieldref o3 c12);
;log(named-fieldref c2 o3 c12);
;log(named-fieldref c1 o3 c12);
;named-fieldset c2 o3 c12 = -(0,120);
;named-fieldset c1 o3 c12 = -(0,12);
;log(fieldref o3 c12);
;log(named-fieldref c2 o3 c12);
;log(named-fieldref c1 o3 c12)
;end
...
log(send o3 m1());
log(named-send c2 o3 m1());
log(named-send c1 o3 m1());
log(named-send c1 o3 m10())
