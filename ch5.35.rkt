...
(define (apply-procedure/k proc1 vals cont try-cont0)
  (cases proc proc1
...
(define-datatype continuation continuation?
  ...
  (try-cont
   ...
   (prev-try-cont continuation?))
  (raise1-cont
   (saved-cont continuation?)))

(define (apply-cont cont val try-cont0)
  (cases continuation cont
    ...
    (zero1-cont (saved-cont)
                (apply-cont saved-cont (bool-val (zero? (expval->num val))) try-cont0))
    (let-exp-cont (vars exps body binding-eval-env body-eval-env saved-cont)
                  (let ((my-env (extend-env (car vars) val body-eval-env)))
                    (if (= (length vars) 1)
                        (value-of/k body my-env saved-cont try-cont0)
                        (value-of/k (car exps) binding-eval-env
                                    (let-exp-cont (cdr vars) (cdr exps) body binding-eval-env my-env saved-cont)
                                    try-cont0))))
    (if-test-cont (exp2 exp3 saved-env saved-cont)
                  (if (expval->bool val)
                      (value-of/k exp2 saved-env saved-cont try-cont0)
                      (value-of/k exp3 saved-env saved-cont try-cont0)))
    (diff1-cont (exp2 saved-env saved-cont)
                (value-of/k exp2 saved-env (diff2-cont val saved-cont) try-cont0))
    (diff2-cont (val1 saved-cont)
                (let ((num1 (expval->num val1))
                      (num2 (expval->num val)))
                  (apply-cont saved-cont (num-val (- num1 num2)) try-cont0)))
    ...
    (rands-cont (rator rands rand-vals saved-env saved-cont)
                (if (null? rands)
                    (let ((proc1 (expval->proc rator)))
                      (apply-procedure/k proc1 (append rand-vals (list val)) saved-cont try-cont0))
                    (value-of/k (car rands) saved-env
    ...
    (get-pair-car-cont (saved-cont)
                       (apply-cont saved-cont (get-pair-car val) try-cont0))
    (get-pair-cdr-cont (saved-cont)
                       (apply-cont saved-cont (get-pair-cdr val) try-cont0))
    (null?-cont (saved-cont)
                (apply-cont saved-cont (is-null? val) try-cont0))
    (list-exp-cont (vals exps saved-env saved-cont)
                   (let ((new-vals (append vals (list val))))
                     (if (null? exps)
                         (apply-cont saved-cont (list->pairs new-vals) try-cont0)
                         (value-of/k (car exps) saved-env
                                     (list-exp-cont new-vals
                                                    ...
                                                    saved-cont)
                                     try-cont0))))
    (try-cont (var handler-exp env cont prev-try-cont)
              (apply-cont cont val prev-try-cont))
    (raise1-cont (saved-cont)
                 (apply-handler val try-cont0))))

(define (apply-handler val cont)
  (cases continuation cont
    (try-cont (var handler-exp saved-env saved-cont prev-try-cont)
    ...
    (else (eopl:printf "unexpected-error"))))
...
(define (value-of-program pgm)
  (cases program pgm
    (a-program (exp1) (value-of/k exp1 (init-env) (end-cont) (end-cont)))))

(define (value-of/k exp env cont try-cont0)
  (cases expression exp
    (const-exp (num) (apply-cont cont (num-val num) try-cont0))
    (var-exp (var) (apply-cont cont (apply-env env var) try-cont0))
    (proc-exp (vars body)
              (apply-cont cont
    ...
    (letrec-exp (p-name p-vars p-body let-body)
                (value-of/k let-body
                            ...
                            try-cont0))
    (zero?-exp (exp1)
               (value-of/k exp1 env (zero1-cont cont) try-cont0))
    (if-exp (exp1 exp2 exp3)
            (value-of/k exp1 env (if-test-cont exp2 exp3 env cont) try-cont0))
    (let-exp (vars exps body)
             (value-of/k (car exps) env
                         (let-exp-cont vars (cdr exps) body env env cont)
                         try-cont0))
    (diff-exp (exp1 exp2)
              (value-of/k exp1 env (diff1-cont exp2 env cont) try-cont0))
    ...
    (cons-exp (exp1 exp2)
              (value-of/k exp1 env
                          ...
                          try-cont0))
    (emptylist-exp ()
                   (apply-cont cont (emptypair-val) try-cont0))
    (car-exp (exp1)
             (value-of/k exp1 env
                         (get-pair-car-cont cont) try-cont0))
    (cdr-exp (exp1)
             (value-of/k exp1 env
    ...
    (try-exp (exp1 var handler-exp)
             (value-of/k exp1 env
                         (try-cont var handler-exp env cont try-cont0)
                         (try-cont var handler-exp env cont try-cont0)))
    (raise-exp (exp1)
               (value-of/k exp1 env
                           (raise1-cont cont)
...
; (run "let fn = proc(x) x in try (fn 5) catch (err) -2")
; (run "let fn = proc(x) try raise 4 catch(err) -1 in try (fn 44) catch (err) -2")